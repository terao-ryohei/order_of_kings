...
out_path = os.path.join(script_dir, '..', 'server', 'drizzle', 'seed_warriors_batch2.sql')

wb = openpyxl.load_workbook(xlsx_path, read_only=False)
ws = wb['武将ALL']

# 武将ALL の全データ行を取得してデータ末尾を特定
...
# Batch2: row 32 to last_data_row (warrior_id 31 to last_data_row-1)
warriors_sql = []
...
# Batch1で60スキル投入済み → Batch2スキルはID 61から
skill_id = 61

for i, row in enumerate(ws.iter_rows(min_row=32, max_row=last_data_row, values_only=True)):
    if row[0] is None:
        continue

    warrior_id = i + 31  # row32=warrior31, row33=warrior32, ...
    name = esc(row[0])
    reading = esc(row[1])
    ...
    reading_val = f"'{reading}'" if reading else "''"
    ...
    warriors_sql.append(
        ...
        f"VALUES ({warrior_id}, '{name}', {reading_val}, {rarity}, "
        f"{atk}, {int_}, {guts}, {pol}, "
...
with open(out_path, 'w', encoding='utf-8') as f:
    f.write('-- BATCH: 2\n')
    f.write(f'-- RANGE: warriors 31-{31 + len(warriors_sql) - 1}\n')
    f.write('-- GENERATED_AT: 2026-03-26\n\n')

    f.write('-- Warriors Batch2\n')
...
print(f"Generated {len(warriors_sql)} warriors (id 31-{30 + len(warriors_sql)})")
print(f"Generated {len(weapon_aptitudes_sql)} weapon_aptitudes")
print(f"Generated {len(skills_sql)} skills (id 61-{60 + len(skills_sql)})")
print(f"Generated {len(warrior_skills_sql)} warrior_skills")
print(f"last_data_row in 武将ALL: {last_data_row}")
